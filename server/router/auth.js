...
router.post('/signup', async (req, res) => {
    ...
    const { name, email, password, cpassword, fathername, address, img, dob, sex } = req.body;

    if (!name || !email || !cpassword || !password || !fathername || !address || !img || !dob || !sex) {
    ...
    try {
        ...
        if (userExists) {
            ...
            return res.status(422).json({ err: "password does not match with confirm password" });
        } else {
            const user = new User({ name, email, password, cpassword, fathername, address, img, dob, sex, role });
            //pre save middleware
            await user.save();
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (userLogin) {
            ...
            res.json({ message: "user login successful",token,isUser:userLogin.role=='user'?true:false });
...
router.get('/update', authenticate, (req, res) => {
    console.log('update address from auth');
    res.send(req.rootUser);
})
